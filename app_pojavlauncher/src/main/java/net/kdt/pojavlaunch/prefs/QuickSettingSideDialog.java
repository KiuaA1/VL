...
import static net.kdt.pojavlaunch.prefs.LauncherPreferences.PREF_GYRO_SMOOTHING;
...
public abstract class QuickSettingSideDialog extends com.kdt.SideDialogView {
    ...
    private Switch mVirtualMouseSwitch, mGyroSmoothSwitch, mSwapHandSwitch;
    ...
    private boolean mOriginalVirtualMouse, mOriginalGyroSmooth, mOriginalSwapHand;
    ...
    private void bindLayout() {
        ...
        mVirtualMouseSwitch = mDialogContent.findViewById(R.id.checkboxVirtualMouse);
        mGyroSmoothSwitch = mDialogContent.findViewById(R.id.checkboxGyroSmooth);
        mSwapHandSwitch = mDialogContent.findViewById(R.id.checkboxSwapHand);
    ...
    private void setupListeners() {
        ...
        mOriginalVirtualMouse = PREF_VIRTUAL_MOUSE_START;
        mOriginalGyroSmooth = PREF_GYRO_SMOOTHING;
        mOriginalSwapHand = PREF_DISABLE_SWAP_HAND;
        ...
        if (mVirtualMouseSwitch != null) mVirtualMouseSwitch.setChecked(mOriginalVirtualMouse);
        if (mGyroSmoothSwitch != null) mGyroSmoothSwitch.setChecked(mOriginalGyroSmooth);
        if (mSwapHandSwitch != null) mSwapHandSwitch.setChecked(mOriginalSwapHand);
        ...
        if (mVirtualMouseSwitch != null) {
            mVirtualMouseSwitch.setOnCheckedChangeListener((buttonView, isChecked) -> {
                PREF_VIRTUAL_MOUSE_START = isChecked;
                mEditor.putBoolean("mouse_start", isChecked);
            });
        }
        if (mGyroSmoothSwitch != null) {
            mGyroSmoothSwitch.setOnCheckedChangeListener((buttonView, isChecked) -> {
                PREF_GYRO_SMOOTHING = isChecked;
                mEditor.putBoolean("gyroSmoothing", isChecked);
                onGyroStateChanged();
            });
        }
        if (mSwapHandSwitch != null) {
            mSwapHandSwitch.setOnCheckedChangeListener((buttonView, isChecked) -> {
